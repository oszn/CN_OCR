...
def draw_bbox(img_path, result, words,color=(255, 0, 0), thickness=2):
    ...
    for point in result:
        ...
        pilimg = Image.fromarray(img_path)

        # PIL图片上打印汉字
        draw = ImageDraw.Draw(pilimg)  # 图片上打印
        font = ImageFont.truetype("D:\\CRNN_Chinese_Characters_Rec-stable\\dbnet\\仿宋_GB2312.ttf", 20, encoding="utf-8")  # 参数1：字体文件路径，参数2：字体大小
        draw.text((point[3][0],point[3][1] ), words[index], (255, 0, 0), font=font)  # 参数1：打印坐标，参数2：文本，参数3：字体颜色，参数4：字体
        img_path = cv2.cvtColor(np.array(pilimg), cv2.COLOR_RGB2BGR)
        index+=1
    return img_path
